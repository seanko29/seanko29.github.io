...
**Anthropic API는 쓸 때마다 과금됩니다. Claude Pro/Max 구독으로는 커버되지 않습니다** —
구독과 API는 청구가 완전히 별개입니다.

회당 대략 이 정도입니다(1,600px 사진 1장 · 환율 1,400원 기준 추정):
...
입력이 약 3,000토큰(사진 1,600 + 프롬프트 1,400), 출력이 사고 토큰 포함
1,000~2,000토큰 정도입니다. **비용의 대부분은 출력과 사고 토큰**이라,
사진 크기를 줄이는 것보다 모델과 생각 깊이를 낮추는 쪽이 훨씬 효과가 큽니다.

리뷰 초안 정도면 Sonnet 5 + 생각 깊이 '낮음'으로 충분합니다.
설정에서 바꿀 수 있습니다(웹은 설정 패널, 로컬은 `.env` 의 `REVIEW_MODEL` / `REVIEW_EFFORT`).

정확한 사용량은 결과 화면 하단의 토큰 수와
